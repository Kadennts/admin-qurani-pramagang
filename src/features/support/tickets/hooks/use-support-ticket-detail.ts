...
import { SUPPORT_TICKET_STATUSES } from "../model/constants";
import {
...
} from "../data/support-tickets.repository";
import type {
  SupportTicketDetail,
...
export function useSupportTicketDetail(ticketId: string, profileName?: string) {
  ...
  // Mendengarkan perubahan live di tabel ticket_replies untuk tiket ini
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          // Saat ada reply baru masuk (dari pengguna lain / realtime),
          // tambahkan ke atas daftar replies (karena urutan DESC)
          const newReply = payload.new as TicketReply & { is_initial?: boolean };
          setReplies((current) => {
            // Hindari duplikasi jika reply sudah ada (misal setelah kita sendiri kirim)
            if (current.some((r) => r.id === newReply.id)) return current;
            return [{ ...newReply, attachments: [] }, ...current];
          });
        },
      )
      .on(
        ...
        (payload) => {
          // Update state tiket secara realtime jika ada perubahan (status, priority, dll)
          setTicket((current) =>
            current ? { ...current, ...(payload.new as Partial<SupportTicketDetail>) } : current,
          );
        },
      )
      .subscribe();

    // Bersihkan subscription saat komponen di-unmount atau ticketId berubah
    return () => {
  ...
  const handleReplySubmit = useCallback(async () => {
    ...
    try {
      ...
      // Tambahkan reply baru ke awal list (newest first)
