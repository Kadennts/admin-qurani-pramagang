...
import { formatTicketDateTime } from "../model/utils";
      ...
      <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-12">
          ...
          <div className="flex-1">
            {state.activeTab === "add_reply" && (
              <div className="space-y-6">
                ...
                {(() => {
                  // Replies diambil DESC (terbaru di index 0, terlama di akhir)
                  // Index terakhir = pesan paling awal = "opened this ticket" (body tiket)
                  const sorted = [...state.replies];
                  const initialReply = sorted.length > 0 ? sorted[sorted.length - 1] : null;
                  // Regular replies = semua kecuali yang paling pertama (paling lama)
                  const regularReplies = sorted.length > 1 ? sorted.slice(0, -1) : [];

                  return (
                    <>
                      {/* ── Pesan pembuka tiket (body awal) ──────────────────────────── */}
                      {/* Selalu muncul tepat di bawah Reply to Ticket box, tidak bergerak */}
                      {initialReply && (
                        <div className="flex flex-col items-start gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                          <div className="flex w-full items-center gap-3">
                            {/* Avatar inisial pembuat tiket */}
                            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700">
                              {initialReply.author?.charAt(0)?.toUpperCase() ?? "U"}
                            </div>
                            <div className="flex flex-1 items-center justify-between">
                              <div className="flex items-center gap-2 text-sm">
                                <span className="font-bold text-slate-800">{initialReply.author}</span>
                                <span className="font-normal text-slate-500">opened this ticket</span>
                              </div>
                              <span className="text-xs text-slate-400">
                                {formatTicketDateTime(initialReply.created_at, "en-GB", profile.timezone || "Asia/Jakarta")}
                              </span>
                            </div>
                          </div>
                          {/* Isi body tiket yang ditulis pembuat saat membuat tiket */}
                          {initialReply.text && (
                            <p className="text-sm whitespace-pre-wrap text-slate-700">{initialReply.text}</p>
                          )}
                        </div>
                      )}

                      {/* ── Reply-reply berikutnya dari admin/staff ──────────────────── */}
                      {regularReplies.map((reply) => (
                        <div
                          key={reply.id}
                          className="flex flex-col items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"
                        >
                          <div className="flex w-full items-center gap-3">
                            {/* Avatar inisial author reply */}
                            ...
                            <div className="flex flex-1 items-center justify-between">
                              <div>
                                ...
                                <span className="ml-1.5 text-xs font-semibold text-slate-400">(Staff)</span>
                              </div>
                              <span className="text-xs text-slate-400">
                                {formatTicketDateTime(reply.created_at, "en-GB", profile.timezone || "Asia/Jakarta")}
                              </span>
                            </div>
                          </div>
                          {reply.text && (
                            <p className="ml-11 text-sm whitespace-pre-wrap text-slate-700">{reply.text}</p>
                          )}
        ...
        >
          ...
          <div>
            ...
            <div className="space-y-4">
              {/* Department: nilai diambil dari data tiket (huruf kapital) */}
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500">Department</span>
                <span className="font-bold uppercase text-slate-800">
                  {state.ticket?.department || "-"}
                </span>
              </div>
              {/* Project: belum tersedia di DB, tampilkan tanda "-" */}
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-500">Project</span>
                <span className="font-medium text-slate-800">-</span>
              </div>
              {/* Service: belum tersedia di DB, tampilkan tanda "-" */}
              <div className="flex items-center justify-between text-sm">
